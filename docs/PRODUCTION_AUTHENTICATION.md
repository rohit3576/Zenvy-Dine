...
- RBAC transport: Firestore remains the source of truth, and Firebase custom claims mirror `role`, `restaurantSlug`, `isActive`, and `permissions` so login can continue when profile reads are blocked by rules during bootstrap.
...
- Every admin also has matching Firebase custom claims for `role`, `restaurantSlug`, `restaurantId`, `isActive`, and `permissions`.
